...
def get_dwell_thresholds(
    ...
    *,
    ...
    """
    return _load_thresholds("DWELL_THRESHOLD", zone_ids, store_id, session=session)
...
def get_queue_length_thresholds(
    ...
    *,
    ...
    """
    thresholds = _load_thresholds("QUEUE_THRESHOLD", zone_ids, store_id, session=session)
...
def get_queue_duration_thresholds(
    ...
    *,
    ...
    dict[str, float | None]
        Mapping of zone_id → threshold_seconds.
    """
    return _load_thresholds("QUEUE_THRESHOLD_DURATION", zone_ids, store_id, session=session)
...
def get_occupancy_severity(
    store_id: str,
    session: Session | None = None,
) -> str:
    """Severity from the matched OCCUPANCY_THRESHOLD rule (default ``warning``)."""
    loaded = _load_occupancy_rule(store_id, session=session)
    if loaded is not None:
        return loaded[1]
    return "warning"
...
def _load_occupancy_rule(
    ...
    *,
    ...
    if session is not None:
        return _load_occupancy_rule_in_session(session, store_id)

    with session_scope() as sess:
        return _load_occupancy_rule_in_session(sess, store_id)


def _load_occupancy_rule_in_session(
    session: Session,
    store_id: str,
) -> tuple[float, str] | None:
    stmt = select(AlertRule).where(
    ...
    *,
    ...
    if session is None:
        with session_scope() as owned:
            return _load_thresholds_from_session(
                owned, rule_type, zone_ids=zone_ids, store_id=store_id
            )
    return _load_thresholds_from_session(
        session, rule_type, zone_ids=zone_ids, store_id=store_id
    )
