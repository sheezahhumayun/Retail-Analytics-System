...
from database.processing_runs import finalize_processing_run, request_processing_run_cancellation
...
def enqueue_processing_run(
    ...
    *,
    ...
    """Insert a pending processing_run row for the inference poller."""
...
def start_recorded_processing(
    ...
    *,
    ...
    """Queue a recorded-video job for the inference worker."""
    return enqueue_processing_run(camera_id, recording_start=recording_start)
...
def start_live_analytics_subprocess(
    ...
    """Launch the continuous live-analytics worker in the inference venv (local dev)."""
...
def stop_live_analytics_subprocess(proc: subprocess.Popen[str] | None) -> None:
    """Terminate the live-analytics worker subprocess (local dev)."""
    global _live_analytics_shutdown_requested
    if proc is None:
        return
...
def kill_processing_runs_for_org(org_id: str) -> int:
    """Request cancellation for in-flight recorded-video jobs in ``org_id``."""
    with session_scope() as session:
        org_camera_ids = {
            camera.id for camera in session.exec(cameras_for_org_stmt(org_id)).all()
        }
        if not org_camera_ids:
            return 0
        killed = request_processing_run_cancellation(
            session,
            camera_ids=org_camera_ids,
            message=ORG_DISABLE_CANCEL_MESSAGE,
        )
    if killed:
        logger.info(
            "Cancelled %d pending processing run(s) for disabled organization %s",
            killed,
            org_id,
        )
    return killed
...
claim_processing_run = enqueue_processing_run
