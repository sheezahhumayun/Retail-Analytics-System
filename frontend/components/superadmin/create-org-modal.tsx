...
import type { CreateOrganizationData } from '@/lib/api/organizations-admin';

interface CreateOrgModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: CreateOrganizationData) => Promise<void>;
}
...
export function CreateOrgModal({ isOpen, onClose, onSave }: CreateOrgModalProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!id.trim()) {
      ...
      newErrors.id = 'Organization id must be 64 characters or fewer';
    }

    if (!name.trim()) {
      newErrors.name = 'Organization name is required';
    } else if (name.trim().length > 255) {
      newErrors.name = 'Organization name must be 255 characters or fewer';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await onSave({ id: id.trim(), name: name.trim() });
      onClose();
    } catch (error) {
      setSaveError(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md max-h-[90vh] bg-card border border-border rounded-lg shadow-lg overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label htmlFor="org-id" className="block text-sm font-medium text-foreground mb-1">
              Organization ID
            </label>
            <input
              id="org-id"
